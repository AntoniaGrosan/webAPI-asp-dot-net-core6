...
namespace CityInfo.API.Controllers
{
    ...
    public class PointsOfInterestController : ControllerBase
    {
        ...
        {
            var cityOfUser = User.Claims.FirstOrDefault(c => c.Type == "city")?.Value;
            if(!await _cityInfoRepository.CityNameMatchesCityId(cityOfUser, cityId))
            {
                return Forbid(); // 403 status: user auth but doesn t have access to smth
            }
